...
class ChamferLoss(Module):
    ...
    def forward(self, pc_source: torch.Tensor, pc_target: torch.Tensor, pred_flow: torch.Tensor) -> torch.Tensor:
        ...
        idx = pointops.knnquery(self.k, pc_target, pc_pred)#[b,n,1]
        nn1 = pointops.grouping(pc_target_t, idx.detach())#[[b,3,n.1]#nn1中的点来自于target，pred是接近target

        dist1 = (pc_pred_t.unsqueeze(3) - nn1).norm(p=self.loss_norm, dim=1).mean(dim=-1)  # nn flow consistency

        idx = pointops.knnquery(self.k, pc_pred, pc_target)#idx是target中的每个点在pred中寻找最近距离的点的索引
