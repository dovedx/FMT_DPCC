...
def visualize_residual_distribution(residual, save_path="./visual_pcd/residual_distribution.png"):
    """
    可视化残差特征分布并保存图像

    参数:
        residual: torch.Tensor, 形状 [N, 128]
        save_path: str, 保存路径 (支持 .png / .pdf)
    """
    assert residual.dim() == 2, "residual 应为 [N, C] 张量"
    data = residual.flatten().detach().cpu().numpy()

    # 统计信息
    mean_val = np.mean(data)
    std_val = np.std(data)
    entropy_approx = -np.sum(np.histogram(data, bins=200, density=True)[0] *
                             np.log2(np.histogram(data, bins=200, density=True)[0] + 1e-9))
    print(f"[INFO] 数据统计: mean={mean_val:.4f}, std={std_val:.4f}, entropy≈{entropy_approx:.2f} bits")

    # 创建图像
    plt.figure(figsize=(6, 4))
    sns.histplot(data, bins=120, kde=True, color='steelblue', stat='density', alpha=0.8)
    plt.title("Distribution of Residual Features", fontsize=13)
    ...
    plt.close()
    print(f"[INFO] 已保存残差分布图到: {save_path}")




class CrossPointTransformer_v2(nn.Module):
    def __init__(self, Cin=32, Cout=32, k=16):
        super().__init__()
        self.k = k

        # 绝对位置编码
        self.abs_pos_mlp = nn.Sequential(
            nn.Linear(3, Cin),
            nn.ReLU(),
            nn.Linear(Cin, Cin)
        )
    ...
    def forward(self, ref: ME.SparseTensor, cur: ME.SparseTensor):
        ref_xyz = ref.C[:, 1:].float()
        cur_xyz = cur.C[:, 1:].float()

      
        ref_xyz = (ref_xyz - ref_xyz.mean(0)) / ref_xyz.std(0)
        cur_xyz = (cur_xyz - cur_xyz.mean(0)) / cur_xyz.std(0)

        # 加入绝对位置编码
        ref_feats = ref.F + self.abs_pos_mlp(ref_xyz)
        cur_feats = cur.F + self.abs_pos_mlp(cur_xyz)

        # KNN 查找
        knn = knn_points(cur_xyz[None], ref_xyz[None], K=self.k)
        idx = knn.idx.squeeze(0)

        k_feats = ref_feats[idx]
        k_xyz = ref_xyz[idx]

        q = cur_feats[:, None, :]
        rel_pos = cur_xyz[:, None, :] - k_xyz
        pos_enc = self.pos_mlp(rel_pos)

        att_input = q - k_feats + pos_enc
        att_logits = self.att_mlp(att_input).squeeze(-1)
        att_weights = torch.softmax(att_logits, dim=-1)

        agg = torch.sum(att_weights[..., None] * k_feats, dim=1)
        out = self.out_mlp(agg)
        out = self.ln(out + cur_feats)

        return ME.SparseTensor(features=out,
                               coordinates=cur.C,
                               coordinate_manager=cur.coordinate_manager,
                               tensor_stride=cur.tensor_stride,
                               device=cur.device)

    
class CrossPointTransformer(nn.Module):
    def __init__(self, Cin=32, Cout=32, k=16):
        super().__init__()
        self.k = k
    ...
    def forward(self, ref: ME.SparseTensor, cur: ME.SparseTensor):
        """
        ref: SparseTensor with feats (N_ref, Cin), coords (N_ref, 4: batch,x,y,z)
        cur: SparseTensor with feats (N_cur, Cin), coords (N_cur, 4: batch,x,y,z)
        """

        # ---- Extract coords & feats ----
        ref_xyz = ref.C[:, 1:].float()  # (N_ref, 3)
        cur_xyz = cur.C[:, 1:].float()  # (N_cur, 3)

        ref_feats = ref.F  # (N_ref, Cin)
        cur_feats = cur.F  # (N_cur, Cin)

        # ---- KNN 查询（假设 batch=1 全部点一起算）----
        knn = knn_points(cur_xyz[None], ref_xyz[None], K=self.k)
        idx = knn.idx.squeeze(0)  # (N_cur, k)

        # Gather KNN features & positions
        k_feats = ref_feats[idx]  # (N_cur, k, Cin)
        k_xyz = ref_xyz[idx]      # (N_cur, k, 3)

        # ---- Construct Query, Key, Relative Pos ----
        q = cur_feats[:, None, :]            # (N_cur, 1, Cin) → broadcast to (N_cur, k, Cin)
        rel_pos = cur_xyz[:, None, :] - k_xyz  # (N_cur, k, 3)
        pos_enc = self.pos_mlp(rel_pos)      # (N_cur, k, Cin)

        # ---- Attention logits & weights ----
        att_input = q - k_feats + pos_enc     # (N_cur, k, Cin)
        att_logits = self.att_mlp(att_input).squeeze(-1)  # (N_cur, k)
        att_weights = torch.softmax(att_logits, dim=-1)    # (N_cur, k)

        # ---- Weighted sum ----
        agg = torch.sum(att_weights[..., None] * k_feats, dim=1)  # (N_cur, Cin)

        # ---- Output transform + residual + LN ----
        out = self.out_mlp(agg)  # (N_cur, Cout)
        out = self.ln(out + cur_feats)  # residual
...
#         neighbor_feats = neighbor_feats.view(N2, self.K, -1)  # (N2, K, C)
        ...
        return predicted_y2, residual_y2,residual,residual_2 
