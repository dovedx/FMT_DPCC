...
class get_model(nn.Module):
    def __init__(self, channels=8,entropy_mode='cond',knn_fixed=True,point_conv=False,scale_wave=False):
        ...
        if self.knn_fixed:
            if scale_wave==False:
                ...
                self.inter_prediction_v2_0 = PointCloudFeaturePredictor(in_channels=64, K=32,point_conv=self.point_conv)
    ...
    def forward(self, f1, f2, device, epoch=99999,idx=0):
        ...
        # print(f'encode down: {elapsed_time_ms:.1f}ms')  # Elapsed: 212.1ms
        ...
        if self.knn_fixed:
            ...
            if self.scale_wave==True:
                if idx==0:
                    predicted_point2, residual ,_= self.inter_prediction_v2_0(ys1[2], ys2[2])
                elif idx==1:
                    predicted_point2, residual ,_= self.inter_prediction_v2_0(ys1[2], ys2[2])
                elif idx==2:
                    predicted_point2, residual ,_= self.inter_prediction_v2_0(ys1[2], ys2[2])
