...
def feature_match_new(point1,xyz_2):
        #context2要在context1中寻找最近邻的坐标，然后将最近邻特征提取出来，形成新的sparse_tensor
        
        N1, C = point1.C[:,1:].shape
        N2, _ = xyz_2.shape
        
        xyz1 =  point1.C[:,1:].unsqueeze(0).float()
        xyz2 =  xyz_2[:,1:].unsqueeze(0).float()
        ...
        grouped_f_ref = index_points_group(point1.F.unsqueeze(0), knn_idx)
        new_feature = torch.sum(weight.view(1, N2, 3, 1) * grouped_f_ref, dim = 2)
        
        # new_point1 =  ME.SparseTensor(new_feature.squeeze(),coordinate_map_key=point2.coordinate_map_key, coordinate_manager=point2.coordinate_manager)
        # new_point1 =ME.SparseTensor(new_feature.squeeze(),coordinates=point2.C, tensor_stride=point2.tensor_stride )
        return new_feature[0]

def feature_match(point1,point2):
        #context2要在context1中寻找最近邻的坐标，然后将最近邻特征提取出来，形成新的sparse_tensor
        ...
        xyz1 =  point1.C[:,1:].unsqueeze(0).float()
        xyz2 =  point2.C[:,1:].unsqueeze(0).float()
        knn_idx = knn_point( 3, xyz1, xyz2 )
...
def sort_by_coor_sum(f, stride=None):
    """
    Sort points by their unique coordinate position .

    Input:
        f: points to be sorted
    Return:
        f_:, sorted f
    """
    if stride is None:
...
def merge_two_frames(f1, f2):
    ...
    # print("f1,f2============>",f1.tensor_stride,f2.tensor_stride ,f1.shape,f2.shape)
    ...
    # print("f1_,f2_============>",f1_.tensor_stride,f2_.tensor_stride ,f1_.shape,f2_.shape)

    
    merged_f = f1_ + f2_
    
    # print("merged_f:",merged_f.shape)
...
# def isin_alternative(out_tensor, in_tensor):
#     # 确保输入是一维张量
#     out_tensor = out_tensor.view(-1)
#     in_tensor = in_tensor.view(-1)
#     # 获取 in_tensor 的唯一值并排序
#     in_unique = torch.unique(in_tensor)
#     in_sorted, _ = torch.sort(in_unique)
#     # 使用 searchsorted 查找 out_tensor 的元素
#     indices = torch.searchsorted(in_sorted, out_tensor)
#     # 检查是否有效（indices 未越界且匹配）
#     valid = (indices < in_sorted.size(0)) & (in_sorted[indices] == out_tensor)
#     return valid
...
def get_target_by_sp_tensor(out, target_sp_tensor):
    with torch.no_grad():
        ...
        # target =isin_alternative(out_tensor=out_sp_tensor_coords_1d, in_tensor=in_sp_tensor_coords_1d)
...
class inter_prediction_RA(nn.Module):
    ...
    def forward(self, f1, f2, stride=8, training=True):
        ...
        # print("compressed_motion2:",compressed_motion2.F)
        ...
        m= self.prune(m, f2)#新加的代码
        motion = m.F
        
        # print("motion ===>",motion.shape,motion)
        ...
        motion = motion.reshape(B, N, C, 3)
        # print("motion==>",motion,motion.size())
        # torch.Size([1, 77348, 64, 3])
        # print("xyz2 shape:",xyz2.shape)
        # print("motion shape:",motion.shape)
        
        
        xyz2_ = (xyz2.unsqueeze(2) + motion).reshape(B, -1, 3)
        # print("xyz2_:",xyz2_.shape)
        
        # xyz2 shape: torch.Size([1, 19687, 3])
        # motion shape: torch.Size([1, 19687, 64, 3])
        # xyz2_: torch.Size([1, 1259968, 3])
        
        #see https://pytorch3d.readthedocs.io/en/latest/modules/ops.html#pytorch3d.ops.knn_points for instruction on "knn_points"
        dist, knn_index1_, __ = knn_points(xyz2_, xyz1, K=3) #xyz2_在xyz1中寻找最近邻
        #knn_index1_:[1x1259968x3x3]
        # print("knn_index1_:",knn_index1_.shape)
        dist += 1e-8
        # print("===>xyz2_, xyz1,dist, knn_index1_",xyz2_.shape, xyz1.shape ,dist.shape, knn_index1_.shape)
        # ===>xyz2_, xyz1,dist, knn_index1_ torch.Size([1, 4950272, 3]) torch.Size([1, 76802, 3]) torch.Size([1, 4950272, 3]) torch.Size([1, 4950272, 3])

        knn_index1_ = knn_index1_.reshape(B, N, C, 3) 
        # print("knn_index1_:",knn_index1_.shape)
        # print("===>point1, knn_index1_:",point1.shape, knn_index1_.shape)
        # ===>point1, knn_index1_: torch.Size([1, 76802, 64]) torch.Size([1, 77348, 64, 3])
        knn_point1_ = index_by_channel(point1, knn_index1_, 3)
        # print("knn_point1_:",knn_point1_.shape)
        dist = dist.reshape(B, N, C, 3)
        weights = 1 / dist
        weights = weights / torch.clamp(weights.sum(dim=3, keepdim=True), min=3) # 3 is the penalty coefficient \alpha for isolated points with far neighbors
        # print("weughts:",weights.shape)
        predicted_point2 = (weights * knn_point1_).sum(dim=3).squeeze(0)
        
        # print("predicted_point2:",predicted_point2)
        
        # print("predicted_point2:",predicted_point2.shape)
...
class inter_prediction(nn.Module):
    ...
    def forward(self, f1, f2, stride=8, training=True):
        ...
        # print("input optical shape:",f1.shape,f2.shape)
...
class UpsampleLayer(nn.Module):
    def __init__(self, input, hidden, output, block_layers, kernel=2):
        ...
        # self.up_context = ME.MinkowskiGenerativeConvolutionTranspose(
        #     in_channels=72,
        #     out_channels=hidden,
        #     kernel_size=kernel,
        #     stride=2,
        ...
        self.block_fuse=self.make_layer(
            InceptionResNet, block_layers,output)
    ...
    def forward(self, x, target_label, adaptive, rho=1, residual=None, lossless=False,context2=None,f1_ref=None):
        ...
        if residual is not None:
            # print("out,residual:",out.F.shape,residual.F.shape)
            stride = out.tensor_stride[0]
            residual = ME.SparseTensor(residual.F, coordinates=residual.C,
                                       coordinate_manager=out.coordinate_manager)
            # print("train mode:",self.training)
            # print("ori residual:",residual.F[1:10],torch.sum(residual.F))
            # print("ori out:",out.F[1:10],torch.sum(out.F))
            ...
            # print("")
            # print("add residual out:",out.F[1:10,0],torch.sum(out.F))
        ...
        # print("out_pruned,out_cls:",out_pruned.shape,out_cls.shape)
        
        # print("diff. ===>",torch.sum(out_pruned.C - f1_ref.C))
        
        if f1_ref!=None:
            f1_ref = ME.SparseTensor(f1_ref.F, coordinates=f1_ref.C,coordinate_manager=out_pruned.coordinate_manager,tensor_stride=out_pruned.tensor_stride)
            out_pruned = out_pruned + f1_ref
            
            out_pruned = self.block_fuse(out_pruned)
        
        # print("out_pruned:",out_pruned.shape)
        return out_pruned, out_cls, target, keep

    
class MV_DeconvWithPruning(nn.Module):
    def __init__(self, input, output):
        super(MV_DeconvWithPruning, self).__init__()
        self.up = ME.MinkowskiGenerativeConvolutionTranspose(
            in_channels=input,
            out_channels=output,
            kernel_size=3,
