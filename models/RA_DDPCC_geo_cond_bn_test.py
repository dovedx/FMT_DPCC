...
class get_model(nn.Module):
    def __init__(self, channels=8,entropy_mode='cond',knn_fixed=True,point_conv =False,fuse_conv=False):
...
#             self.cond = True
    ...
    def update_buffer(self, reconstructed, video_idx, frame_idx):
        ...
        if frame_idx == (gop_start + self.gop_size - 1):  # 当前帧是 GOP 的最后一帧
            print("一个gop结束,开始更新buff",frame_idx)
            torch.cuda.empty_cache()  # 清理 PyTorch 缓存
            self.clean_buffer(video_idx, gop_start)
            
    def clean_buffer(self, video_idx, prev_gop_start):
        ...
        print("prev_gop_start:",prev_gop_start)
        prev_gop_last = prev_gop_start + self.gop_size - 1
        keys_to_keep = [(video_idx, prev_gop_last)]
    ...
    def forward(self, current_f,indices, device, epoch=99999):
        ...
        if ref1 is None and ref2 is None:  # I帧
            ...
            if self.knn_fixed:
                ...
                print("layer IMNN GPU time: {:.3f} ms".format(self.start.elapsed_time(self.end)))
                motion_bits=0
            else:
