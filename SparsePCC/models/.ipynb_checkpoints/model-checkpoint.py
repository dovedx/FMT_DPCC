...
from SparsePCC.extension.metrics import pc_error
...
from SparsePCC.data_utils.inout import write_ply_o3d
...
class PCCModel(torch.nn.Module):
    def __init__(self, channels=32, kernel_size=3, block_layers=3, stage=8, scale=1, enc_type='pooling'):
        ...
        if enc_type=='pooling': 
            ...
            from SparsePCC.models.autoencoder import AutoEncoder
            self.downsampler = AutoEncoder(
                in_channels=1, channels=channels, out_channels=channels, kernel_size=kernel_size, 
                block_layers=block_layers)
            latent_channels=channels
        if stage==1: from SparsePCC.models.decoder import UpSampler1Stage as UpSampler
        elif stage==0: from SparsePCC.models.decoder_mask import UpSamplerMask as UpSampler
        else: from SparsePCC.models.decoder import UpSampler8Stage as UpSampler
    ...
    def forward(self, ground_truth, training=True):
        out_set_list = []
        for idx in range(self.scale):#[0,1,2,3]
            
            x = quantize_sparse_tensor(ground_truth, factor=1/(2**idx), quant_mode='floor')
            print('DBG!!!', idx, '\t',x.shape, len(x), x.C.max().cpu().numpy(), x.C.min().cpu().numpy(), x.C.max().cpu().numpy() - x.C.min().cpu().numpy())
            print("factor:",1/(2**idx))
            ...
            print("out_set:",out_set["out_cls_list"][0].C.shape,out_set["out_cls_list"][0].F.shape)
            print("groundtruth:",out_set["ground_truth_list"][0].C.shape,out_set["ground_truth_list"][0].F.shape)
            print("\n")
