...
class BasicCoder():
    ...
    def test(self, x, bin_dir, dec_dir, voxel_size=1, posQuantscale=1):
        start = time.time()
...
class LossyCoderDense():
    ...
    def test(self, x_raw , bin_dir, dec_dir, voxel_size=1, posQuantscale=1, scale_AE=1, scale_SR=2, psnr_resolution=1023):
        start = time.time()
        ...
        results = { 'num_points_raw':num_points_raw, 'file_size':bits, 'bpp':bpp, 'num_points':points_dec.shape[0],
                'enc_time':enc_time, 'all_enc_time':all_enc_time, 'dec_time':dec_time, 'all_dec_time':all_dec_time}
        results.update(psnr_results)
